...
class LaboratoryWorkflow:
    def __init__(self, research_topic, openai_api_key, max_steps=100, num_papers_lit_review=5, agent_model_backbone=f"{DEFAULT_LLM_BACKBONE}", notes=list(), human_in_loop_flag=None, compile_pdf=True, mlesolver_max_steps=3, papersolver_max_steps=5):
        ...
        ##########################################
        self.num_ref_papers = 20         # 控制最终文献综述中引用的核心论文数量
        self.review_total_steps = 15     # num steps to take if overridden 评审过程的最大允许迭代次数（当review_override=True时生效） 
        self.arxiv_num_summaries = 5     # 每次arXiv搜索返回的摘要数量
...
if __name__ == "__main__":
    ...
    task_notes_LLM = [
        ...
        "note": "Prioritize papers about: 1) Video-based 3D reconstruction 2) SAM 2: Segment Anything in Images and Videos and its applications in 3D reconstruction 3) Temporal consistency in multi-view geometry 4) YOLOv12: Attention-Centric Real-Time Object Detectors and its application for 3D reconstruction. Filter by top conferences like CVPR/ICCV/ECCV/NeurlIPS/ICML/AAAI publications after 2023(inclusive)."},
        
        {"phases": ["plan formulation"],
         "note": f"You should come up with 2-5 plans of experiments for choose."},

        {"phases": ["plan formulation"],
         "note": f"You should seperate the whole experiments into mini tasks/stages/phases, each task only concentrate on one goal"},

        {"phases": ["plan formulation"],
         "note": f"You should come up to approach and data for each phase from reliable source like top conference paper, diferent phase could share the same dataset"},

        {"phases": ["plan formulation"],
         "note": f"You should justify the practicality of each combination of data and approach for each phase"},
    ]
